...
(function () {
  ...
  Scene_Boot.prototype.loadSystemImages = function () {
    ...
    if (Liquidize.MadeWithMV.ShowMV) {
      ImageManager.loadParallax("main-bg");
    }
    if (Liquidize.MadeWithMV.ShowCustom) {
      ImageManager.loadParallax(Liquidize.MadeWithMV.CustomImage);
    }
  };
  ...
  Scene_Splash.prototype.createSplashes = function () {
    if (Liquidize.MadeWithMV.ShowMV) {
      this._mvSplash = new Sprite(
        ImageManager.loadParallax(Liquidize.MadeWithMV.MVImage)
      );
  ...
  Scene_Splash.prototype.gotoTitleOrTest = function () {
    ...
    if (DataManager.isBattleTest()) {
      ...
      if (document.cookie.split("isDisSound=")[1]) {
        $gameSystem._isDisSound =
          document.cookie.split("isDisSound=")[1].split(";")[0] === "true";
      }

      $gameSystem.imgSound = "btn_sound";
      if ($gameSystem._isDisSound) {
        $gameSystem.imgSound = "btn_unsound";
      }
      if ($gameSystem._isDisSound) {
